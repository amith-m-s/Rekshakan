...
function ResidentApp({ session, onLogout }: { session: Session; onLogout: () => void }) {
  ...
  return <SafeAreaView style={s.app} edges={['top', 'bottom']}>
    <View style={s.topbar}><View><Text style={s.hello}>Hello, {session.user.name.split(' ')[0]}</Text><Text style={s.muted}>Resident safety centre</Text></View><View style={s.badges}><View style={s.demoPill}><Text style={s.demoPillText}>CALIFORNIA</Text></View><View style={s.livePill}><View style={s.liveDot}/><Text style={s.liveText}>LIVE</Text></View></View></View>
...
function Home({ incident, requests, shelters, refreshing, refresh, go, session }: { incident: Incident | null; requests: HelpRequest[]; shelters: Shelter[]; refreshing: boolean; refresh: () => void; go: (tab: Tab) => void; session: Session }) {
  ...
  return <Page refreshing={refreshing} refresh={refresh}>
    <View style={s.locationStrip}><Text style={s.locationStripIcon}>⌖</Text><View style={s.flex}><Text style={s.locationStripTitle}>Santa Cruz Mountains, California</Text><Text style={s.locationStripText}>Unified demonstration location for mobile and console</Text></View></View>
...
const s = StyleSheet.create({
  ...
  topbar: { paddingHorizontal: 20, paddingVertical: 14, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: 'white', borderBottomWidth: 1, borderBottomColor: '#e6ece9' }, hello: { fontSize: 20, fontWeight: '800', color: '#102a22' }, muted: { color: '#687a73', lineHeight: 19 }, badges: { alignItems: 'flex-end', gap: 5 }, demoPill: { backgroundColor: '#e8efff', borderRadius: 20, paddingHorizontal: 9, paddingVertical: 4 }, demoPillText: { color: '#315ea8', fontWeight: '800', fontSize: 8, letterSpacing: .5 }, livePill: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: '#e4f6ef', borderRadius: 20, paddingHorizontal: 11, paddingVertical: 5 }, liveDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: '#0ca678' }, liveText: { color: '#08775a', fontWeight: '800', fontSize: 10 }, banner: { backgroundColor: '#fff1db', padding: 10 }, bannerText: { color: '#794600', textAlign: 'center', fontWeight: '600', fontSize: 12 },
  locationStrip: { backgroundColor: '#e9f1ff', borderRadius: 15, padding: 13, flexDirection: 'row', alignItems: 'center', gap: 11, borderWidth: 1, borderColor: '#cadaf7' }, locationStripIcon: { color: '#315ea8', fontSize: 25, fontWeight: '800' }, locationStripTitle: { color: '#1c467f', fontWeight: '800', fontSize: 14 }, locationStripText: { color: '#60779a', fontSize: 11, marginTop: 2 },
