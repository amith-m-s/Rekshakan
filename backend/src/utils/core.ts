...
export const severityLevel = (score: number) => score <= 20 ? 'LOW' : score <= 40 ? 'MODERATE' : score <= 60 ? 'HIGH' : score <= 80 ? 'VERY_HIGH' : 'CRITICAL';
export const priorityLevel = (score: number) => score >= 80 ? 'CRITICAL' : score >= 60 ? 'URGENT' : score >= 35 ? 'HIGH' : score >= 15 ? 'NORMAL' : 'LOW';
export const log = (level: string, event: string, data: Record<string, unknown> = {}) => console.log(JSON.stringify({ timestamp: now(), level, event, ...data }));
